        ...
        <main class="flex-1 w-full xl:ml-[260px] p-4 sm:p-6 lg:p-8 space-y-6 max-w-none min-w-0 overflow-y-auto">
            ...
            <div class="mb-6 flex items-center">
                ...
                <h1 class="text-2xl font-semibold text-[#1D1D1D]">
                    Detail Skrining Pasien ({{ optional(optional(optional($skrining)->pasien)->user)->name ?? 'N/A' }})
                </h1>
            </div>
            ...
            @else
                <div class="rounded-2xl bg-white p-6 shadow">
                    ...
                    <input type="hidden" id="tinggi_badan"
                        value="{{ optional($skrining->kondisiKesehatan)->tinggi_badan ?? '' }}">
                    <input type="hidden" id="berat_badan"
                        value="{{ optional($skrining->kondisiKesehatan)->berat_badan_saat_hamil ?? '' }}">

                    <div class="overflow-hidden rounded-xl border border-gray-200">
                        <div class="grid grid-cols-1 sm:grid-cols-3">
                            <div class="border-b border-gray-200 p-4 text-sm bg-pink-50 font-semibold">Informasi</div>
                            <div class="sm:col-span-2 border-b border-gray-200 p-4 text-sm bg-pink-50 font-semibold">
                                Data</div>
                            ...
                            <div class="sm:col-span-2 border-t border-gray-200 p-4 text-sm">
                                {{ optional($skrining->created_at)->format('d F Y') }}</div>
                            ...
                            <div class="sm:col-span-2 p-4 text-sm">
                                ...
                                @if ($pkm && (bool) $pkm->is_mandiri)
                                    ...
                                    <div class="text-xs text-[#6B7280]">Bidan Mandiri — Kec.
                                        {{ $pkm->kecamatan ?? '-' }}</div>
                                @else
                                    @php($name = $pkm->nama_puskesmas ?? null)
                                    <div class="font-medium">
                                        {{ $name ? (\Illuminate\Support\Str::startsWith(strtolower($name), 'puskesmas') ? $name : 'Puskesmas ' . $name) : '-' }}
                                    </div>
                            ...
                            <div class="border-t border-gray-200 p-4 text-sm font-semibold">Indeks Masa Tubuh (IMT)
                            </div>
                            <div class="sm:col-span-2 border-t border-gray-200 p-4 text-sm">
                                <input id="imt_result" type="text" readonly
                                    class="w-full rounded-md border px-3 py-2 text-sm"
                                    value="{{ optional($skrining->kondisiKesehatan)->imt !== null ? number_format(optional($skrining->kondisiKesehatan)->imt, 2) : '' }}"
                                    placeholder="Akan terisi otomatis oleh sistem" />
                            </div>

                            <div class="border-t border-gray-200 p-4 text-sm font-semibold">Status IMT</div>
                            <div class="sm:col-span-2 border-t border-gray-200 p-4 text-sm">
                                <span id="imt_category"
                                    class="{{ optional($skrining->kondisiKesehatan)->status_imt ? '' : 'text-gray-400' }}">{{ optional($skrining->kondisiKesehatan)->status_imt ?? '-' }}</span>
                            </div>

                            <div class="border-t border-gray-200 p-4 text-sm font-semibold">Anjuran Kenaikan BB</div>
                            <div class="sm:col-span-2 border-t border-gray-200 p-4 text-sm">
                                {{ optional($skrining->kondisiKesehatan)->anjuran_kenaikan_bb ?? '-' }}</div>
                            ...
                            <div class="border-t border-gray-200 p-4 text-sm font-semibold">Mean Arterial Pressure (MAP)
                            </div>
                            <div class="sm:col-span-2 border-t border-gray-200 p-4 text-sm">
                ...
                <div class="mt-8 rounded-2xl bg-white p-6 shadow">
                    ...
                    <div class="overflow-hidden rounded-xl border border-gray-200">
                        <div class="grid grid-cols-1 sm:grid-cols-3">
                            ...
                            <div class="sm:col-span-2 border-b border-gray-200 p-4 text-sm font-medium text-gray-900">
                                RT {{ optional($skrining->pasien)->rt ?? '-' }} / RW
                                {{ optional($skrining->pasien)->rw ?? '-' }}
                            </div>
                ...
                <div class="mt-8 rounded-2xl bg-white p-6 shadow">
                    ...
                    <div class="overflow-hidden rounded-xl border border-gray-200">
                        <div class="grid grid-cols-1 sm:grid-cols-3">
                            ...
                            <div class="border-t border-gray-200 p-4 text-sm font-semibold">Riwayat Penyakit Pasien
                            </div>
                            <div class="sm:col-span-2 border-t border-gray-200 p-4 text-sm">
                            ...
                            <div class="border-t border-gray-200 p-4 text-sm font-semibold">Riwayat Penyakit Keluarga
                            </div>
                            <div class="sm:col-span-2 border-t border-gray-200 p-4 text-sm">
                            ...
                            <div class="sm:col-span-2 border-t border-gray-200 p-4 text-sm">
                                {{ $skrining->kesimpulan ?? '-' }}</div>

                            <div class="border-t border-gray-200 p-4 text-sm font-semibold">Catatan</div>
                            <div class="sm:col-span-2 border-t border-gray-200 p-4 text-sm">{{ 'Belum ada catatan.' }}
                            </div>
                        </div>
                    </div>
