        ...
        <main class="flex-1 w-full xl:ml-[260px] p-4 sm:p-6 lg:p-8 space-y-6 max-w-none min-w-0 overflow-y-auto">
            ...
            <section class="space-y-4">
                ...
                <div class="bg-white rounded-2xl border border-[#E9E9E9] p-2 sm:p-4">
                    ...
                    <!-- Filter Modal -->
                    <div x-cloak x-show="showFilter" x-transition:enter="transition ease-out duration-200"
                        ...
                        <div class="mt-4 flex flex-wrap items-center gap-2">
                            ...
                            @if (request('nik'))
                                <span
                                    ...
                                    NIK: {{ request('nik') }}
                                    <a href="{{ route('rs.skrining.index', array_merge(request()->except('nik'))) }}"
                            ...
                            @if (request('nama'))
                                <span
                                    class="inline-flex items-center gap-1 px-3 py-1 bg-pink-100 text-pink-700 rounded-full text-xs">
                                    Nama: {{ request('nama') }}
                                    <a href="{{ route('rs.skrining.index', array_merge(request()->except('nama'))) }}"
                                        class="hover:text-pink-900">×</a>
                                </span>
                            @endif
                            @if (request('tanggal_dari') || request('tanggal_sampai'))
                                <span
                                    ...
                                    <a href="{{ route('rs.skrining.index', array_merge(request()->except(['tanggal_dari', 'tanggal_sampai']))) }}"
                                        class="hover:text-pink-900">×</a>
                                </span>
                            @endif
                            @if (request('risiko'))
                                <span
                                    ...
                                    <a href="{{ route('rs.skrining.index', array_merge(request()->except('risiko'))) }}"
                                        class="hover:text-pink-900">×</a>
                                </span>
                            @endif
                            <a href="{{ route('rs.skrining.index') }}"
                                ...
                                @forelse($skrinings as $r)
                                    ...
                                    <tr>
                                        <td class="px-3 py-3 font-medium text-[#1D1D1D]">
                                            {{ ($skrinings->firstItem() ?? 0) + $loop->index }}
                                        </td>
                                        ...
                                        <td class="px-3 py-3">
                                            <div class="flex items-center gap-2">
                                                <a href="{{ route('rs.pasien.show', $pas->id) }}"
                                                    class="px-4 py-1 rounded-full border border-[#D9D9D9] text-[#1D1D1D] text-xs hover:bg-gray-50 transition">View
                                                    Pasien</a>
                                                ...
                                                <a href="{{ route('rs.skrining.show', $skr->id) }}"
                                                    class="px-4 py-1 rounded-full border border-[#D9D9D9] text-[#1D1D1D] text-xs hover:bg-gray-50 transition">Detail
                                                    Pengecekan</a>
                                            </div>
                                        </td>
                                    </tr>
