...
<body class="bg-[#FFF7FC] min-h-screen overflow-x-hidden">
    <div class="flex min-h-screen" x-data="{ openSidebar: false }">
        ...
        <main class="flex-1 w-full xl:ml-[260px] p-4 sm:p-6 lg:p-8 space-y-6 max-w-none min-w-0 overflow-y-auto">
            ...
            @if (session('info'))
                <div
                    class="flex items-start gap-2 rounded-xl border border-blue-100 bg-blue-50 px-3 py-2 text-xs sm:text-sm text-blue-800">
                    <span class="mt-0.5">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="2">
                            <circle cx="12" cy="12" r="10" />
            ...
            <section class="bg-white rounded-2xl border border-[#E9E9E9] p-3 sm:p-4 space-y-4">
                ...
                <form action="{{ route('rs.pasien-nifas.store') }}" method="POST" id="formPasienNifas"
                    class="space-y-4">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <div class="flex gap-2">
                                <input type="tel" name="nik" id="nik" maxlength="16" inputmode="numeric"
                                    pattern="[0-9]*"
                                    class="block w-full rounded-lg border border-[#E5E5E5] bg-white px-3 py-2 text-xs sm:text-sm text-[#1D1D1D] shadow-sm focus:border-[#E91E8C] focus:ring-1 focus:ring-[#E91E8C]/30 transition-colors"
                                    placeholder="Masukkan NIK 16 digit" value="{{ old('nik') }}" required>
                                <button type="button" id="btnCekNik" data-url="{{ route('rs.pasien-nifas.cek-nik') }}"
                    ...
                    <div id="statusRisikoWrapper" class="">
                        ...
                        <select name="status_risiko_manual" id="status_risiko_manual"
                            class="block w-full rounded-lg border border-[#E5E5E5] bg-white px-3 py-2 text-xs sm:text-sm text-[#1D1D1D] shadow-sm focus:border-[#E91E8C] focus:ring-1 focus:ring-[#E91E8C]/30 transition-colors">
                            <option value="">Pilih Status Risiko</option>

                            <option value="beresiko"
                                {{ old('status_risiko_manual') == 'beresiko' ? 'selected' : '' }}>
                                Beresiko
                            </option>
                            ...
                            <option value="unknown" {{ old('status_risiko_manual') == 'unknown' ? 'selected' : '' }}>
                                Belum ada data skrining / Tidak Diketahui
                            </option>
                        </select>
                    ...
                    <div id="wilayah-wrapper" data-url-provinces="{{ route('wilayah.provinces') }}"
                        data-url-regencies="{{ url('/wilayah/regencies') }}"
                        data-url-districts="{{ url('/wilayah/districts') }}"
                        data-url-villages="{{ url('/wilayah/villages') }}" data-prov="{{ old('provinsi') }}"
                        data-kab="{{ old('kota') }}" data-kec="{{ old('kecamatan') }}"
                        data-kel="{{ old('kelurahan') }}" class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="provinsi" id="provinsi"
                                    class="block w-full rounded-lg border border-[#E5E5E5] bg-white px-3 py-2 text-xs sm:text-sm text-[#1D1D1D] shadow-sm focus:border-[#E91E8C] focus:ring-1 focus:ring-[#E91E8C]/30 transition-colors"
                                    required>
                                    <option value="">{{ old('provinsi') ? 'Memuat…' : 'Pilih Provinsi' }}
                                    </option>
                                </select>
                                @error('provinsi')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            ...
                            <div>
                                ...
                                <select name="kota" id="kabupaten"
                                    class="block w-full rounded-lg border border-[#E5E5E5] bg-white px-3 py-2 text-xs sm:text-sm text-[#1D1D1D] shadow-sm focus:border-[#E91E8C] focus:ring-1 focus:ring-[#E91E8C]/30 transition-colors"
                                    required>
                                    <option value="">{{ old('kota') ? 'Memuat…' : 'Pilih Kota/Kabupaten' }}
                                    </option>
                                </select>
                                @error('kota')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="kecamatan" id="kecamatan"
                                    class="block w-full rounded-lg border border-[#E5E5E5] bg-white px-3 py-2 text-xs sm:text-sm text-[#1D1D1D] shadow-sm focus:border-[#E91E8C] focus:ring-1 focus:ring-[#E91E8C]/30 transition-colors"
                                    required>
                                    <option value="">{{ old('kecamatan') ? 'Memuat…' : 'Pilih Kecamatan' }}
                                    </option>
                                </select>
                                @error('kecamatan')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            ...
                            <div>
                                ...
                                <select name="kelurahan" id="kelurahan"
                                    class="block w-full rounded-lg border border-[#E5E5E5] bg-white px-3 py-2 text-xs sm:text-sm text-[#1D1D1D] shadow-sm focus:border-[#E91E8C] focus:ring-1 focus:ring-[#E91E8C]/30 transition-colors"
                                    required>
                                    <option value="">{{ old('kelurahan') ? 'Memuat…' : 'Pilih Kelurahan' }}
                                    </option>
                                </select>
                                @error('kelurahan')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <textarea name="domisili" id="domisili" rows="3"
                            class="block w-full rounded-lg border border-[#E5E5E5] bg-white px-3 py-2 text-xs sm:text-sm text-[#1D1D1D] shadow-sm focus:border-[#E91E8C] focus:ring-1 focus:ring-[#E91E8C]/30 transition-colors"
                            placeholder="Contoh: Jl. Margonda Raya" required>{{ old('domisili') }}</textarea>
                        @error('domisili')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="tempat_lahir" id="tempat_lahir"
                                ...
                                placeholder="Contoh: Depok" value="{{ old('tempat_lahir') }}">
                            @error('tempat_lahir')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    {{-- Status Perkawinan --}}
                    <div>
                        <label class="block text-[11px] font-semibold text-[#666666] mb-1">Status Perkawinan</label>
                        <select name="status_perkawinan" id="status_perkawinan"
                            ...
                            <option value="0" {{ old('status_perkawinan') == '0' ? 'selected' : '' }}>Belum
                                Menikah
                            </option>
                            <option value="1" {{ old('status_perkawinan') == '1' ? 'selected' : '' }}>Menikah
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-[11px] font-semibold text-[#666666] mb-1">Pembiayaan
                                Kesehatan</label>
                            <select name="pembiayaan_kesehatan" id="pembiayaan_kesehatan"
                                class="block w-full rounded-lg border border-[#E5E5E5] bg-white px-3 py-2 text-xs sm:text-sm">
                                <option value="">Pilih Pembiayaan</option>
                                <option value="Pribadi"
                                    {{ old('pembiayaan_kesehatan') == 'Pribadi' ? 'selected' : '' }}>Pribadi</option>
                                <option value="BPJS Kesehatan (JKN)"
                                    {{ old('pembiayaan_kesehatan') == 'BPJS Kesehatan (JKN)' ? 'selected' : '' }}>BPJS
                                    Kesehatan (JKN)</option>
                                <option value="Asuransi Lainnya"
                                    {{ old('pembiayaan_kesehatan') == 'Asuransi Lainnya' ? 'selected' : '' }}>Asuransi
                                    Lainnya</option>
                            </select>
