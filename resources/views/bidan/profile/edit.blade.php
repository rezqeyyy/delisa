        ...
        <div class="rounded-2xl bg-white p-6 shadow">
            ...
            <div class="w-full flex justify-center mb-6">
                <div class="relative w-32 h-32 sm:w-36 sm:h-36">
                    ...
                    <div
                        class="absolute inset-0 rounded-full ring-4 ring-pink-100 bg-pink-50 flex items-center justify-center shadow">
                    ...
                    <img id="avatarPreview" src="{{ $user->photo ? Storage::url($user->photo) . '?t=' . time() : '#' }}"
                        alt="Avatar"
            ...
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-2 sm:gap-3">
                <label for="photoInput"
                    class="cursor-pointer inline-flex items-center justify-center gap-2 bg-[#B9257F] hover:bg-[#a31f70] text-white px-4 py-2 rounded-full shadow transition-colors">
                ...
                @if ($user->photo)
                    <form action="{{ route('bidan.profile.photo.destroy') }}" method="POST" class="inline-flex">
                        ...
                        @method('DELETE')
                        <button type="submit"
                            class="inline-flex items-center justify-center gap-2 bg-white border border-rose-200 text-rose-600 hover:bg-rose-50 px-4 py-2 rounded-full transition-colors"
            ...
            {{-- FORM DATA UTAMA --}}
            <form id="profileForm" action="{{ route('bidan.profile.update') }}" method="POST"
                enctype="multipart/form-data" class="space-y-5 mt-6">
                @csrf
                @method('PUT')
                <div class="w-full max-w-xl md:max-w-5xl mx-auto">
                    <label class="block text-sm font-medium mb-2">Nama</label>
                    <input type="text" name="name" value="{{ old('name', $user->name) }}"
                        class="w-full rounded-xl border border-gray-200 px-4 py-2.5 focus:ring-2 focus:ring-pink-200 focus:outline-none transition-shadow"
                        required>
                </div>

                <div class="w-full max-w-xl md:max-w-5xl mx-auto">
                    <label class="block text-sm font-medium mb-2">Nomor Izin Praktek</label>
                    <input type="text" name="nomor_izin_praktek"
                        value="{{ old('nomor_izin_praktek', $bidan->nomor_izin_praktek) }}"
                        class="w-full rounded-xl border border-[#D9D9D9] bg-[#F5F5F5] px-4 py-2.5 focus:outline-none cursor-not-allowed text-gray-500"
                        readonly required>
                </div>

                <div class="w-full max-w-xl md:max-w-5xl mx-auto">
                    <label class="block text-sm font-medium mb-2">Puskesmas Binaan</label>

                    <input type="text"
                        value="{{ optional($puskesmasBinaan)->nama_puskesmas
                            ? optional($puskesmasBinaan)->nama_puskesmas . ' (Kec. ' . optional($puskesmasBinaan)->kecamatan . ')'
                            : '-' }}"
                        class="w-full rounded-xl border border-[#D9D9D9] bg-[#F5F5F5] px-4 py-2.5 focus:outline-none cursor-not-allowed text-gray-500"
                        readonly>
                </div>
                ...
                <div class="w-full max-w-xl md:max-w-5xl mx-auto">
                    ...
                    <input type="password" name="password"
                        class="w-full rounded-xl border border-[#D9D9D9] px-4 py-2.5 focus:ring-2 focus:ring-[#B9257F]/30 focus:outline-none transition-shadow"
                ...
                <div class="w-full max-w-xl md:max-w-5xl mx-auto flex justify-end pt-2">
                    <button type="submit"
                        class="inline-flex items-center gap-2 bg-[#B9257F] hover:bg-[#a31f70] text-white px-6 sm:px-7 py-3 rounded-full shadow transition-all hover:shadow-lg">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
