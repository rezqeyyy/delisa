    ...
    <div class="lg:ml-[260px] mx-auto max-w-8xl px-3 sm:px-6 lg:px-8 py-6 lg:py-8">
        ...
        <div class="rounded-2xl bg-white p-6 shadow">
            ...
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-2 sm:gap-3">
                <label for="photo" class="cursor-pointer inline-flex items-center justify-center gap-2 bg-[#B9257F] hover:bg-[#a31f70] text-white px-4 py-2 rounded-full shadow">
                ...
                <input id="photo" name="photo" type="file" accept="image/*" class="hidden" form="profileForm" />

                <form action="{{ route('bidan.profile.photo.destroy') }}" method="POST" class="inline-flex">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="inline-flex items-center justify-center gap-2 bg-white border border-rose-200 text-rose-600 hover:bg-rose-50 px-4 py-2 rounded-full">
            ...
            <form id="profileForm" action="{{ route('bidan.profile.update') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                @csrf
                @method('PUT')
                <div class="w-full max-w-xl md:max-w-5xl mx-auto">
                    <label class="block text-sm font-medium mb-2">Nama</label>
                    <input type="text" name="name" value="{{ old('name', $user->name) }}" class="w-full rounded-xl border border-gray-200 px-4 py-2.5 focus:ring-2 focus:ring-pink-200 focus:outline-none" required>
                </div>

                <div class="w-full max-w-xl md:max-w-5xl mx-auto">
                    <label class="block text-sm font-medium mb-2">Nomor Izin Praktek</label>
                    <input type="text" name="nomor_izin_praktek" value="{{ old('nomor_izin_praktek', $bidan->nomor_izin_praktek) }}" class="w-full rounded-xl border border-[#D9D9D9] bg-[#F5F5F5] px-4 py-2.5 focus:outline-none cursor-not-allowed" readonly required>
                </div>

                <div class="w-full max-w-xl md:max-w-5xl mx-auto">
                    <label class="block text-sm font-medium mb-2">Password Baru</label>
                    <input type="password" name="password" class="w-full rounded-xl border border-[#D9D9D9] px-4 py-2.5 focus:ring-2 focus:ring-[#B9257F]/30 focus:outline-none">
                </div>

                <div class="w-full max-w-xl md:max-w-5xl mx-auto">
                    <label class="block text-sm font-medium mb-2">Konfirmasi Password</label>
                    <input type="password" name="password_confirmation" class="w-full rounded-xl border border-[#D9D9D9] px-4 py-2.5 focus:ring-2 focus:ring-[#B9257F]/30 focus:outline-none">
                </div>

                <div class="w-full max-w-xl md:max-w-5xl mx-auto flex justify-end pt-2">
                    <button type="submit" class="inline-flex items-center gap-2 bg-[#B9257F] hover:bg-[#a31f70] text-white px-6 sm:px-7 py-3 rounded-full shadow">
