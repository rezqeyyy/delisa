    ...
    <div class="flex min-h-screen" x-data="{ openSidebar: false }">
        ...
        <main class="flex-1 w-full xl:ml-[260px] p-4 sm:p-6 lg:p-8 space-y-6 max-w-none min-w-0 overflow-y-auto">
            ...
            <div class="mt-8 rounded-2xl bg-white p-6 shadow">
                ...
                <div class="overflow-hidden rounded-xl border border-gray-200">
                    <div class="grid grid-cols-1 sm:grid-cols-3">
                        <div class="border-b border-gray-200 p-4 text-l bg-pink-50 font-semibold">Informasi</div>
                        <div class="sm:col-span-2 border-b border-gray-200 p-4 text-l bg-pink-50 font-semibold">Data</div>

                        {{-- No. Telepon --}}
                        <div class="border-t border-gray-200 p-4 text-l font-semibold">No. Telepon</div>
                        <div class="sm:col-span-2 border-b border-gray-200 p-4 text-l font-medium text-gray-900">
                            {{ optional(optional($skrining->pasien)->user)->phone ?? optional($skrining->pasien)->no_telepon ?? '-' }}
                        </div>

                        {{-- No. JKN --}}
                        <div class="border-t border-gray-200 p-4 text-l font-semibold">No. JKN</div>
                        <div class="sm:col-span-2 border-b border-gray-200 p-4 text-l font-medium text-gray-900">
                            {{ optional($skrining->pasien)->no_jkn ?? '-' }}
                        </div>

                        {{-- Provinsi --}}
                        <div class="border-t border-gray-200 p-4 text-l font-semibold">Provinsi</div>
                        <div class="sm:col-span-2 border-b border-gray-200 p-4 text-l font-medium text-gray-900">
                            {{ optional($skrining->pasien)->PProvinsi ?? '-' }}
                        </div>

                        {{-- Kabupaten/Kota --}}
                        <div class="border-t border-gray-200 p-4 text-l font-semibold">Kabupaten/Kota</div>
                        <div class="sm:col-span-2 border-b border-gray-200 p-4 text-l font-medium text-gray-900">
                            {{ optional($skrining->pasien)->PKabupaten ?? '-' }}
                        </div>

                        {{-- Kecamatan --}}
                        <div class="border-t border-gray-200 p-4 text-l font-semibold">Kecamatan</div>
                        <div class="sm:col-span-2 border-b border-gray-200 p-4 text-l font-medium text-gray-900">
                            {{ optional($skrining->pasien)->PKecamatan ?? '-' }}
                        </div>

                        {{-- Kelurahan/Wilayah --}}
                        <div class="border-t border-gray-200 p-4 text-l font-semibold">Kelurahan/Wilayah</div>
                        <div class="sm:col-span-2 border-b border-gray-200 p-4 text-l font-medium text-gray-900">
                            {{ optional($skrining->pasien)->PWilayah ?? '-' }}
                        </div>

                        {{-- RT / RW --}}
                        <div class="border-t border-gray-200 p-4 text-l font-semibold">RT / RW</div>
                        <div class="sm:col-span-2 border-b border-gray-200 p-4 text-l font-medium text-gray-900">
                            RT {{ optional($skrining->pasien)->rt ?? '-' }} / RW {{ optional($skrining->pasien)->rw ?? '-' }}
                        </div>

                        {{-- Kode Pos --}}
                        <div class="border-t border-gray-200 p-4 text-l font-semibold">Kode Pos</div>
                        <div class="sm:col-span-2 border-b border-gray-200 p-4 text-l font-medium text-gray-900">
            ...
            {{-- SECTION 3: Hasil Skrining dan Rekomendasi --}}
            <div class="mt-8 rounded-2xl bg-white p-6 shadow">
