    ...
    <main class="ml-0 md:ml-[260px] flex-1 min-h-screen flex flex-col p-4 sm:p-6 lg:p-8 space-y-6">
        <div class="flex-1">
            <header class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
                <div>
                    <h1 class="text-[22px] sm:text-[28px] font-bold leading-tight text-[#000000]">Daftar Pengajuan Akun</h1>
            ...
            <section class="flex flex-col sm:flex-row sm:items-center gap-3 mt-4 sm:mt-6 mb-4 sm:mb-6">
                <form action="{{ route('dinkes.akun-baru') }}" method="GET" class="flex w-full sm:w-auto items-center gap-3">
                    ...
                    <button class="px-5 py-2 rounded-full bg-[#B9257F] text-white text-sm font-medium hover:bg-[#a31f70] transition">
                        Search
                    </button>
                </form>
            </section>
                    ...
                    <article class="bg-white rounded-2xl shadow p-4">
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                ...
                                <div class="mt-1 text-xs"><span class="text-[#7C7C7C]">Role:</span> {{ $req->role->nama_role ?? '-' }}</div>
                            </div>
                            <div class="flex flex-col gap-2">
                                <form action="{{ route('dinkes.akun-baru.approve', $req->id) }}" method="POST"
                                      data-confirm="Terima pengajuan akun {{ $req->name }}?">
                                    @csrf
                                    <button class="px-3 py-1 rounded-full bg-[#A3E4D7] text-[#007965] text-xs font-medium hover:opacity-90 transition w-full">
                                        ✓ Terima
                                    </button>
                                </form>
                                ...
                                <form action="{{ route('dinkes.akun-baru.reject', $req->id) }}" method="POST"
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <button class="px-3 py-1 rounded-full bg-[#B9257F] text-white text-xs font-medium hover:bg-[#a31f70] transition w-full">
                                        ✕ Tolak
                                    </button>
                                </form>
                            </div>
            ...
            <section class="hidden md:block bg-white rounded-2xl shadow-md overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-[760px] w-full text-sm text-left border-collapse">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex justify-center flex-wrap gap-2">
                                        <form action="{{ route('dinkes.akun-baru.approve', $req->id) }}" method="POST"
                                              data-confirm="Terima pengajuan akun {{ $req->name }}?">
                                            @csrf
                                            <button class="px-3 py-1 rounded-full bg-[#A3E4D7] text-[#007965] text-xs font-medium hover:opacity-90 transition">
                                                ✓ Terima
                                            </button>
                                        </form>
                                        ...
                                        <form action="{{ route('dinkes.akun-baru.reject', $req->id) }}" method="POST"
                                            ...
                                            @csrf
                                            @method('DELETE')
                                            <button class="px-3 py-1 rounded-full bg-[#B9257F] text-white text-xs font-medium hover:bg-[#a31f70] transition">
                                                ✕ Tolak
                                            </button>
                                        </form>
                                    </div>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="py-6 text-center text-[#7C7C7C]">Tidak ada pengajuan akun.</td>
                            </tr>
                        @endforelse
                        </tbody>
                    </table>
