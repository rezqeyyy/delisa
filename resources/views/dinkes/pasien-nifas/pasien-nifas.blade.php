...
<body class="bg-[#F5F5F5] font-[Poppins] text-[#000000CC]">
    @php
        // Label dinamis untuk opsi sort "Prioritas"
        $prioritySortLabel = 'Prioritas (Hitam → Ungu → Merah → Hijau)';
        ...
        switch ($priority ?? '') {
            case 'hitam':
                ...
                break;
            case 'periode':
                $prioritySortLabel = 'Prioritas Ungu — Dalam Periode KF';
                break;
            case 'merah':
        ...
        /**
         * Helper (di view): mapping badge class untuk "Sisa Waktu"
         * Penting: class harus statis (bukan dari controller) agar aman dari purge.
         */
        $getSisaBadgeClass = function ($row) {
            $priorityLevel = (int) ($row->priority_level ?? 9);

            $badgeClass = match ($priorityLevel) {
                1 => 'bg-black text-white', // Telat
                2 => 'bg-indigo-600 text-white', // Dalam periode
                3 => 'bg-red-500 text-white', // Sisa 1–3
                4 => 'bg-amber-400 text-black', // Sisa 4–6
                5 => 'bg-green-500 text-white', // Sisa >= 7
                6 => 'bg-gray-200 text-gray-700', // Selesai/meninggal/invalid
                default => 'bg-gray-200 text-gray-700',
            };

            if (($row->status_kf ?? null) === 'INVALID') {
                $badgeClass = 'bg-orange-500 text-white';
            }

            // Untuk meninggal: tampil teks merah (bukan pill)
            if (!empty($row->is_meninggal)) {
                $badgeClass =
                    'inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap text-red-600 font-semibold';
            }
                                    ...
                                    <div class="p-4 space-y-3">
                                        ...
                                        <div class="space-y-1">
                                            ...
                                            <select name="sort" id="sort-select"
                                                ...
                                                <option value="nama_asc" @selected(($sort ?? '') === 'nama_asc')>Nama A → Z
                                                </option>
                                                <option value="nama_desc" @selected(($sort ?? '') === 'nama_desc')>Nama Z → A
                                                </option>
                                                <option value="kf_terbaru" @selected(($sort ?? '') === 'kf_terbaru')>KF Terbaru
                                                </option>
                                                <option value="kf_terlama" @selected(($sort ?? '') === 'kf_terlama')>KF Terlama
                                                </option>
                                            </select>
                                        </div>

                                        {{-- Filter Status Pasien (Meninggal / Tidak) --}}
                                        <div class="space-y-1">
                                            <p class="text-xs font-semibold text-[#6B7280]">Status Pasien</p>
                                            <select name="death" id="death-filter"
                                                class="w-full px-3 py-2 rounded-xl border border-[#D9D9D9]
               text-xs sm:text-sm focus:outline-none focus:ring-1
               focus:ring-[#B9257F]/40 bg-white">
                                                <option value="" @selected(empty($death))>Semua Pasien
                                                </option>
                                                <option value="hidup" @selected(($death ?? '') === 'hidup')>Tidak Meninggal
                                                </option>
                                                <option value="meninggal" @selected(($death ?? '') === 'meninggal')>Hanya Pasien
                                                    Meninggal</option>
                                            </select>
                                        </div>
                                        ...
                                        <div class="space-y-1">
                                            ...
                                            <select name="priority" id="priority-filter"
                                                class="w-full px-3 py-2 rounded-xl border border-[#D9D9D9]
                                                       text-xs sm:text-sm focus:outline-none focus:ring-1
                                                       focus:ring-[#B9257F]/40 bg-white">
                                                <option value="" @selected(empty($priority))>Semua Warna
                                                    Prioritas</option>
                                                <option value="hitam" @selected(($priority ?? '') === 'hitam')>Hitam — Terlambat
                                                </option>
                                                <option value="periode" @selected(($priority ?? '') === 'periode')>Ungu — Dalam
                                                    Periode KF</option>
                                                <option value="merah" @selected(($priority ?? '') === 'merah')>Merah — Sisa 1–3
                                                    Hari</option>
                                                <option value="kuning" @selected(($priority ?? '') === 'kuning')>Kuning — Sisa 4–6
                                                    Hari</option>
                ...
                <section class="md:hidden mt-3 space-y-3">
                    @forelse ($rows as $idx => $row)
                        @php
                            ...
                            $badgeClassSisa = $getSisaBadgeClass($row);
                        @endphp
                            ...
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0 space-y-1.5">
                                    ...
                                    <div class="text-xs">
                                        ...
                                        <span class="font-medium">{{ $row->puskesmas_nama ?? '—' }}</span>
                                    </div>
                                    ...
                                    <div class="text-xs flex items-center gap-2 mt-1.5">
                                        ...
                                        @if (!empty($row->is_meninggal))
                                            <span
                                                class="{{ $badgeClassSisa }} text-[11px]">{{ $sisaLabel }}</span>
                                        @else
                                            <span
                                                class="inline-flex items-center px-3 py-1 rounded-full text-[11px] font-semibold {{ $badgeClassSisa }}">
                                                {{ $sisaLabel }}
                                            </span>
                                        @endif
                                    </div>
                        ...
                        <table class="w-full text-sm text-left border-collapse">
                            <thead class="bg-[#F5F5F5] text-[#7C7C7C] border-b border-[#D9D9D9]">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 font-semibold text-center w-[160px]">Sisa Waktu</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr class="border-b border-[#E9E9E9] hover:bg-[#F9F9F9] transition">
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="flex items-center justify-center">
                                                @if (!empty($row->is_meninggal))
                                                    <span class="{{ $badgeClassSisa }}">{{ $sisaLabel }}</span>
                                                @else
                                                    <span
                                                        class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap {{ $badgeClassSisa }}">
                                                        {{ $sisaLabel }}
                                                    </span>
                                                @endif
                                            </div>
