    ...
    <main class="ml-0 md:ml-[260px] flex-1 min-h-screen flex flex-col p-4 sm:p-6 lg:p-8 space-y-6">
        <div class="flex-1">
            <header class="w-full space-y-3 sm:space-y-4">
                <div>
                    <h1 class="text-[22px] sm:text-[28px] font-bold leading-tight text-[#000000]">Data Pasien Nifas</h1>
                    <p class="text-xs sm:text-sm text-[#7C7C7C]">Kelola Data Pasien Nifas Anda</p>
                </div>

                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                    <section class="flex items-center gap-3">
                        <form action="{{ route('dinkes.pasien-nifas') }}" method="GET" class="flex w-full md:w-auto items-center gap-3">
                    ...
                    <a href="#"
                       class="inline-flex items-center justify-center gap-2 rounded-xl bg-[#B61E7B] px-4 py-2 text-sm font-semibold text-white hover:bg-[#B9257F] transition">
                        <img src="{{ asset('icons/Iconly/Regular/Outline/Paper Download.svg') }}" alt="Download" class="w-5 h-5">
                    ...
                    <article class="rounded-2xl bg-white shadow p-4">
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                ...
                                <div class="text-xs mt-1"><span class="text-[#7C7C7C]">Role:</span> {{ $row->role_penanggung }}</div>
                                <div class="text-xs mt-1">
                                    <span class="text-[#7C7C7C]">TTL:</span>
                                    @php
                                        $ttl = [];
                                        if ($row->tempat_lahir) $ttl[] = $row->tempat_lahir;
                                        if ($row->tanggal_lahir) $ttl[] = \Carbon\Carbon::parse($row->tanggal_lahir)->translatedFormat('d F Y');
                                    @endphp
                                    {{ implode(', ', $ttl) }}
                                </div>
                            </div>
                            <div class="flex flex-col gap-2">
                                <form class="form-delete" action="{{ route('dinkes.pasien-nifas.destroy', $row->id) }}" method="POST"
            ...
            <section class="hidden md:block mt-2 rounded-2xl overflow-hidden border border-[#EDEDED] bg-white">
                <div class="overflow-x-auto">
                    <table class="min-w-[900px] w-full text-sm text-left border-collapse">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-6 text-center text-[#7C7C7C]">Tidak ada data pasien nifas.</td>
                            </tr>
                        @endforelse
                        </tbody>
                    </table>
