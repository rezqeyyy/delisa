        ...
        <section class="flex-1">
            <div class="bg-white rounded-2xl shadow-md px-4 sm:px-8 md:px-12 py-8 sm:py-12 max-w-xl sm:max-w-2xl md:max-w-3xl mx-auto">
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-2 sm:gap-3 mb-8">
                    ...
                    <form id="removePhotoForm" action="{{ route('dinkes.profile.photo.destroy') }}" method="POST" class="hidden">
                        @csrf
                        @method('DELETE')
                    </form>
                </div>
                ...
                {{-- Form profil --}}
                <form id="profileForm" action="{{ route('dinkes.profile.update') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                    @csrf
                    @method('PUT')
