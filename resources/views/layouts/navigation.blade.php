@php
    // Ambil role user yang sedang login (aman untuk null)
    ...
    // Prefix untuk active state (misal: 'puskesmas.')
    $activePrefix = explode('.', $dashboardRoute)[0] . '.';
@endphp
...
<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link :href="route($dashboardRoute)" :active="str_starts_with(request()->route()?->getName(), $activePrefix)">
                        {{ __('Dashboard') }}
                    </x-nav-link>
                    ...
                    @if($role === 'puskesmas')
                        ...
                        <x-nav-link :href="route('puskesmas.pasien-nifas')" :active="request()->routeIs('puskesmas.pasien-nifas*')">
                            {{ __('Pasien Nifas') }}
                        </x-nav-link>
                    @endif

                    {{-- Tambahkan menu khusus role lain di sini jika diperlukan --}}
                </div>
            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route($dashboardRoute)" :active="str_starts_with(request()->route()?->getName(), $activePrefix)">
                {{ __('Dashboard') }}
            ...
            @if($role === 'puskesmas')
                ...
                <x-responsive-nav-link :href="route('puskesmas.pasien-nifas')" :active="request()->routeIs('puskesmas.pasien-nifas*')">
                    {{ __('Pasien Nifas') }}
                </x-responsive-nav-link>
            @endif
        </div>
