...
<aside id="sidebar" aria-label="Navigasi RS"
    ...
    <div class="shrink-0">
        <div class="flex items-center justify-between gap-3 p-6 border-b border-[#CAC7C7]">
            <a href="{{ route('rs.dashboard') }}" class="inline-flex items-center gap-3">
        ...
        <nav class="mt-4 px-3 pb-4 overflow-y-auto max-h-[calc(100vh-150px)] space-y-4">
            ...
            <div class="space-y-2">
                ...
                <a href="{{ route('rs.dashboard') }}" class="{{ $is('rs.dashboard*') ? $active : $idle }}">
                    <img src="{{ asset('icons/Group 36729.png') }}" class="w-4 h-4 transition group-hover:brightness-0 group-hover:invert" alt="Dashboard">
                    <span>Dashboard</span>
                </a>

                <a href="{{ route('rs.skrining.index') }}" class="{{ $is('rs.skrining.*') ? $active : $idle }}">
