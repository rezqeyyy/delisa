...
@php
    ...
    $idle = 'flex items-center gap-3 px-6 py-3 text-[#4B4B4B] hover:bg-[#F5F5F5] rounded-xl transition';
    $is = fn($pattern) => request()->routeIs($pattern);
@endphp

<aside id="sidebar" aria-label="Navigasi Pasien"
    {{ $attributes->merge(['class' => 'fixed top-0 left-0 z-50 h-screen w-[260px] bg-white shadow-lg flex flex-col transform transition-transform duration-200 -translate-x-full lg:translate-x-0']) }}>
    <div class="shrink-0">
    ...
    <form method="POST" action="{{ route(\Illuminate\Support\Facades\Route::has('logout.pasien') ? 'logout.pasien' : 'logout') }}" class="p-3 border-t border-[#F0F0F0]">
        @csrf
        <button type="submit" class="w-full text-left flex items-center gap-3 px-4 py-3 rounded-xl text-[#7C7C7C] hover:bg-[#F3F3F3]">
