...
<section class="bg-white rounded-2xl p-5 shadow-md relative">
  <div class="flex items-center gap-3">
    ...
    <span class="ml-2 text-xs px-2 py-0.5 rounded-full bg-[#F5F5F5] text-[#4B4B4B]">
      Outcome: {{ ['pe'=>'Pre-eklampsia','dirujuk'=>'Dirujuk','meninggal'=>'Meninggal'][$filters['outcome'] ?? 'pe'] }}
    </span>
  ...
  <div id="axFilterPanel"
    ...
    <form method="GET" action="{{ route('dinkes.analytics') }}" class="grid grid-cols-2 gap-4">
      ...
      <div>
        ...
        <div class="grid grid-cols-2 gap-2">
          <input type="number" name="age_min" value="{{ request('age_min', 10) }}" class="w-full border rounded-md px-3 py-2 text-sm">
          <input type="number" name="age_max" value="{{ request('age_max', 60) }}" class="w-full border rounded-md px-3 py-2 text-sm">
        </div>
      </div>
      ...
      <div>
        ...
        <div class="grid grid-cols-2 gap-2">
          <input type="number" name="imt_min" value="{{ request('imt_min', 10) }}" class="w-full border rounded-md px-3 py-2 text-sm">
          <input type="number" name="imt_max" value="{{ request('imt_max', 60) }}" class="w-full border rounded-md px-3 py-2 text-sm">
        </div>
      </div>
      ...
      <div class="col-span-2 flex items-center justify-end gap-2 pt-2 border-t">
        <a href="{{ route('dinkes.analytics') }}" class="border border-[#CAC7C7] rounded-md px-3 py-2 text-sm">Reset</a>
  ...
  <div class="mt-4 grid grid-cols-12 gap-6">
    <div class="col-span-12 lg:col-span-4 bg-[#FAFAFA] rounded-xl p-4">
      ...
      <div class="mt-2 text-sm">Outcome rate: <span class="font-semibold tabular-nums">{{ $rateY }}%</span></div>
    </div>
    ...
    <div class="col-span-12 lg:col-span-8">
      ...
      <div class="grid md:grid-cols-2 gap-3">
        @forelse($top as $it)
          <a href="{{ route('dinkes.analytics.var', ['key' => $it['key']]) }}"
            ...
            @else
              <div class="text-xl font-bold mt-1">OR = {{ $it['or'] }}</div>
