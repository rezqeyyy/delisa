...
<aside id="sidebar" aria-label="Navigasi Puskesmas"
    ...
    <div class="shrink-0">
        <div class="flex items-center justify-between gap-3 p-6 border-b border-[#CAC7C7]">
            <a href="{{ route('puskesmas.dashboard') }}" class="inline-flex items-center gap-3">
        ...
        <nav class="mt-4 px-3 pb-4 overflow-y-auto max-h-[calc(100vh-150px)] space-y-4">
            <div class="space-y-2">
                ...
                <a href="{{ route('puskesmas.dashboard') }}"
                    class="{{ $is('puskesmas.dashboard*') ? $active : $idle }}">
                ...
                <a href="{{ route('puskesmas.skrining.index') }}"
                    class="{{ $is('puskesmas.skrining*') ? $active : $idle }}">
                ...
                <a href="{{ route('puskesmas.rujukan.index') }}"
                    class="{{ $is('puskesmas.rujukan*') ? $active : $idle }}">
                    <img src="{{ asset('icons/Iconly/Regular/Light/Document.svg') }}"
                        class="w-4 h-4 transition group-hover:brightness-0 group-hover:invert" alt="Rujukan">
                    <span>Rujukan</span>
                </a>

                <a href="{{ route('puskesmas.pasien-nifas.index') }}"
                    class="{{ $is('puskesmas.pasien-nifas*') ? $active : $idle }}">
