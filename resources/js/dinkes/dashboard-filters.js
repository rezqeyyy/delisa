    ...
    if (form) {
        ...
        if (reset) {
            reset.addEventListener("click", () => {
                ...
                form.querySelectorAll('input[type="date"], select').forEach(
                    (el) => {
                        el.value = "";
                    },
                );
                ...
                form.querySelectorAll(
                    'input[name="riwayat_penyakit_ui[]"]',
                ).forEach((cb) => {
                    cb.checked = false;
                });
                ...
                // 3) Auto submit setelah reset
                form.submit();

                // 4) Tutup modal supaya efek langsung terlihat
                close();
            });
        }
    }
