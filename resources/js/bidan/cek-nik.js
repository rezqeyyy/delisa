document.addEventListener('DOMContentLoaded', function() {
    ...
    function autofillForm(pasien) {
        ...
        // Data Wilayah: isi dropdown bertingkat berdasarkan NAMA wilayah
        if (window.WilayahCascade) {
            ...
            if (wrapper) {
                wrapper.setAttribute('data-prov', pasien.provinsi || '');
                wrapper.setAttribute('data-kab',  pasien.kota || '');
                wrapper.setAttribute('data-kec',  pasien.kecamatan || '');
                wrapper.setAttribute('data-kel',  pasien.kelurahan || '');
            }
        }
        ...
        function mapPembiayaan(v){ if(!v) return v; const s=String(v).toLowerCase().trim(); if(s.includes('bpjs')||s.includes('jkn')) return 'BPJS Kesehatan'; if(s.includes('pribadi')||s.includes('umum')||s.includes('cash')||s.includes('tunai')) return 'Pribadi'; if(s.includes('asuransi')) return 'Asuransi Lain'; return 'Asuransi Lain'; }
        function mapStatusPerkawinan(v){ if(typeof v === 'boolean') return v ? '1' : '0'; if(v===1||v==='1') return '1'; if(v===0||v==='0') return '0'; const s=String(v||'').toLowerCase().trim(); if(!s) return ''; if(s.includes('menikah')||s.includes('kawin')||s==='married') return '1'; if(s.includes('belum')||s==='single'||s.includes('tidak kawin')) return '0'; return ''; }

        // Data Pribadi
        setFieldValue('tempat_lahir', pasien.tempat_lahir);
        setFieldValue('tanggal_lahir', normalizeDate(pasien.tanggal_lahir));
        setFieldValue('status_perkawinan', mapStatusPerkawinan(pasien.status_perkawinan)); // expects '1' or '0'
        setFieldValue('pekerjaan', pasien.pekerjaan);
        setFieldValue('pendidikan', pasien.pendidikan);

        // Data Kesehatan
        setFieldValue('pembiayaan_kesehatan', mapPembiayaan(pasien.pembiayaan_kesehatan));
    ...
    function setFieldValue(fieldId, value) {
        ...
        if (field.tagName === 'SELECT') {
            ...
            if (byValue) field.value = byValue.value; else {
                const byText = opts.find(opt => opt.textContent.trim().toLowerCase() === val.toLowerCase());
                if (byText) field.value = byText.value;
            }
        } else {
            field.value = val;
        }
        const wilayahIds = ['provinsi','kabupaten','kecamatan','kelurahan'];
        if (wilayahIds.includes(fieldId)) {
