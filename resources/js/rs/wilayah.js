...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function selectByValueOrText(sel, selectedValueOrName) {
    ...
    // 2) Fallback: cocokan berdasarkan text (NAMA) - case-insensitive
    const byText = Array.from(sel.options)
      .find(opt => opt.textContent.trim().toLowerCase() === selectedStr.toLowerCase());
    if (byText) {
  ...
  // Expose setter untuk pengisian berantai via data nama (dipakai oleh cek NIK)
  window.WilayahCascade = {
    setByNames: async ({ prov = '', kab = '', kec = '', kel = '' } = {}) => {
      try {
        setLoading(provSel, true);
        const provData = await fetchJson(API.prov).catch(() => []);
        fillSelect(provSel, provData, prov, 'Pilih Provinsi');
        setLoading(provSel, false);
        ...
        setLoading(kabSel, true);
        const kabData = await fetchJson(API.kab(provId)).catch(() => []);
        fillSelect(kabSel, kabData, kab, 'Pilih Kota/Kabupaten');
        setLoading(kabSel, false);
        ...
        setLoading(kecSel, true);
        const kecData = await fetchJson(API.kec(kabId)).catch(() => []);
        fillSelect(kecSel, kecData, kec, 'Pilih Kecamatan');
        setLoading(kecSel, false);
        ...
        setLoading(kelSel, true);
        const kelData = await fetchJson(API.kel(kecId)).catch(() => []);
        fillSelect(kelSel, kelData, kel, 'Pilih Kelurahan');
        setLoading(kelSel, false);
      } catch (e) {
        console.error('WilayahCascade.setByNames error:', e);
      }
    }
  };
