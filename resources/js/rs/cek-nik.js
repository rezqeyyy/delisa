...
document.addEventListener("DOMContentLoaded", function () {
    ...
    btnCekNik.addEventListener("click", async function () {
        ...
        try {
            ...
            if (data.found) {
                ...
                function setSelectByValueOrText(selId, val) {
                    ...
                    if (opt) {
                        ...
                        console.warn(
                            `⚠️ Option not found for ${selId} with value: ${v}`,
                        );
                    }
                }

                function mapStatusPerkawinan(v) {
                    // FIX: kalau backend kirim boolean/angka
                    if (v === true) return "Menikah";
                    if (v === false) return "Belum Menikah";
                    ...
                    // FIX: handle string "true"/"false" atau "1"/"0"
                    if (s === "true" || s === "1") return "Menikah";
                    if (s === "false" || s === "0") return "Belum Menikah";
                ...
                function mapPembiayaan(v) {
                    const s = String(v || "")
                        ...
                        .trim();
                    if (!s) return "";
                    if (s.includes("bpjs") || s.includes("jkn"))
                        return "BPJS Kesehatan (JKN)";
                    if (
                        s.includes("umum") ||
                        s.includes("pribadi") ||
                        s.includes("tunai") ||
                        ...
                        return "Pribadi";
                    if (s.includes("asuransi") || s.includes("insurance"))
                ...
                if (!statusPerkawinan || !String(statusPerkawinan).trim()) {
                    ...
                    if (umur !== null) {
                        ...
                        statusPerkawinan =
                            umur >= 20 ? "Menikah" : "Belum Menikah";
                        console.log(
                            `📊 Status perkawinan diprediksi dari umur: ${umur} tahun → ${statusPerkawinan}`,
                        );
                    }
                }
                ...
                setSelectByValueOrText(
                    "status_perkawinan",
                    mapStatusPerkawinan(statusPerkawinan),
                );
                setSelectByValueOrText(
                    "pembiayaan_kesehatan",
                    mapPembiayaan(pasien.pembiayaan_kesehatan),
                );
                setSelectByValueOrText(
                    "golongan_darah",
                    mapGolongan(pasien.golongan_darah),
                );
                ...
                if (pasien.has_skrining) {
                    ...
                    showStatusRisikoCard(
                        pasien.status_type,
                        pasien.status_risiko,
                    );
                } else {
                    ...
                    showAlert(
                        "info",
                        "Pasien belum memiliki data skrining. Silakan pilih status risiko secara manual.",
                    );
                }
            } else {
            ...
            showAlert(
                "error",
                "Terjadi kesalahan saat mengecek NIK: " + error.message,
            );
        } finally {
            // Re-enable button
